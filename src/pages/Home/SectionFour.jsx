...
const SectionFour = () => (
  <section className="w-full flex flex-col md:flex-row items-center bg-[#ede7df] -mt-32 border-t border-mainText justify-between">
    <img
      src={imgB}
      className="object-cover h-[50vh] w-full border-l border-mainText md:hidden"
    />
    <div className="md:w-2/3 w-full p-8 flex flex-col md:flex-row items-center justify-center gap-x-28">
      ...
      <div className="flex flex-col items-center md:items-start text-center md:text-left md:-mt-10 -mt-28">
        ...
        <div className="font-barlow text-xl text-mainText mb-8 italic font-medium">
          {texts.sectionFour.subtitle}
        </div>
        {texts.sectionFour.paragraphs.map((p, i) => (
          <p
            key={i}
            className={`text-justify font-barlow text-base text-mainText max-w-md leading-7${
              i > 0 ? " mt-4" : ""
            }`}
          >
            {p}
    ...
    <div className="w-1/3 md:flex items-center justify-end hidden">
      <img
        src={imgB}
        className="object-cover h-screen w-full border-l border-mainText"
      />
    </div>
  </section>
);
